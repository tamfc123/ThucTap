...
function useProductCreateViewModel() {
  ...
  useGetAllApi<GuaranteeResponse>(
    ...
    (guaranteeListResponse) => {
      const selectList: SelectOption[] = guaranteeListResponse.content.map(
        (item) => ({
          value: String(item._id),
          label: item.name,
        })
  ...
  useGetAllApi<SpecificationResponse>(
    ...
    (specificationListResponse) => {
      const selectList: SelectOption[] = specificationListResponse.content.map(
        (item) => ({
  ...
  const transformImages = (
    ...
    return uploadedImageResponses.map((img, index) => ({
      id: null,
      name: img.name,
      path: img.path,
  ...
  const handleFormSubmit = form.onSubmit(async (formValues) => {
    const createProduct = (uploadedImages?: UploadedImageResponse[]) => {
      const images: ImageRequest[] = uploadedImages
        ? transformImages(uploadedImages)
        : [];

      const requestBody: ProductRequest = {
        ...formValues,
        status: Number(formValues.status), // <--- ép kiểu number
      ...
      createApi.mutate(requestBody, {
        onSuccess: async (productResponse) => {
          ...
          const sortedTags = productResponse.tags
            .sort((a, b) => a.name.localeCompare(b.name))
            .map((tag) => String(tag._id) + "#ORIGINAL");
          form.setFieldValue("tags", sortedTags);
        },
        onError: (error) => {
          NotifyUtils.simpleFailed("Tạo sản phẩm không thành công");
        },
      });
    };
    ...
    if (imageFiles.length > 0) {
      try {
        const uploadedImages = await uploadMultipleImagesApi.mutateAsync(
          imageFiles
        );
        createProduct(uploadedImages);
      } catch (error) {
