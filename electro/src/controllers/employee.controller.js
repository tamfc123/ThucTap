import Employee from "../models/Employee.js"

export const getAll = async (req, res, next) => {
    try {
      const { page = 1, size = 10, sort = "createdAt", search } = req.query
      const query = search ? { code: { $regex: search, $options: "i" } } : {}

      const items = await Employee.find(query)
        .sort(sort)
        .limit(size * 1)
        .skip((page - 1) * size)
        .populate("user office department jobType jobLevel jobTitle")

      const total = await Employee.countDocuments(query)

      res.json({
        content: items,
        totalElements: total,
        totalPages: Math.ceil(total / size),
        size: items.length,
      })
    } catch (error) {
      next(error)
    }
  }

  export const getById = async (req, res, next) => {
    try {
      const item = await Employee.findById(req.params.id).populate("user office department jobType jobLevel jobTitle")
      if (!item) {
        return res.status(404).json({ message: "Employee not found" })
      }
      res.json(item)
    } catch (error) {
      next(error)
    }
  }

  export const create = async (req, res, next) => {
    try {
      const item = new Employee(req.body)
      await item.save()
      res.status(201).json(item)
    } catch (error) {
      next(error)
    }
  }

  export const update = async (req, res, next) => {
    try {
      const item = await Employee.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true })
      if (!item) {
        return res.status(404).json({ message: "Employee not found" })
      }
      res.json(item)
    } catch (error) {
      next(error)
    }
  }

  export const deleteMany = async (req, res, next) => {
    try {
      const { ids } = req.body
      await Employee.deleteMany({ _id: { $in: ids } })
      res.json({ message: "Employees deleted successfully" })
    } catch (error) {
      next(error)
    }
  }

export default {
  getAll,
  getById,
  create,
  update,
  deleteMany,
}