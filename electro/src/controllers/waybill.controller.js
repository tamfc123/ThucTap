...
export const create = async (req, res, next) => {
  try {
    // 1. Tách 'orderId' ra khỏi phần còn lại của body
    const { orderId, ...restOfBody } = req.body;
    
    // 2. Tạo mã code
    const code = "WB" + Date.now();

    // 3. Chuẩn bị dữ liệu đúng cho Model
    const waybillData = {
      ...restOfBody, // Gồm weight, height, note...
      code: code,
      order: orderId, // <-- Gán giá trị của 'orderId' (từ frontend) cho trường 'order' (cho backend)
    };

    // 4. Lưu dữ liệu đã chuẩn bị
    const item = new Waybill(waybillData);
    await item.save();

    res.status(201).json(item);
  } catch (error) {
    next(error); // Bắt lỗi validation
  }
};
