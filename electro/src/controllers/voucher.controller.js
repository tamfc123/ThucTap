...
export const getAll = async (req, res, next) => {
    try {
      ...
      const items = await Voucher.find(query)
        .sort(sort)
        .limit(size * 1)
        .skip((page - 1) * size)
  ...
  export const create = async (req, res, next) => {
    try {
      const item = new Voucher(req.body)
      await item.save()
  ...
  export const update = async (req, res, next) => {
    try {
      const item = await Voucher.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true })
      if (!item) {
        return res.status(404).json({ message: "Voucher not found" })
      }
      res.json(item)
  ...
  export const deleteMany = async (req, res, next) => {
    try {
      ...
      res.json({ message: "Vouchers deleted successfully" })
    } catch (error) {
      next(error)
    }
  }
